...
public class GymScreen implements Screen {

    private MySportsGame game;
    private BitmapFont font;

    float dumbbellX = 200, dumbbellY = 200;

    public GymScreen(MySportsGame game) {
        this.game = game;
        font = new BitmapFont();
    }
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isTouched()) {
            dumbbellX = Gdx.input.getX();
            dumbbellY = Gdx.graphics.getHeight() - Gdx.input.getY();
        }

        game.batch.begin();
        font.getData().setScale(LanguageStrings.BUTTON_SCALE);
        font.draw(game.batch, "🏋", dumbbellX, dumbbellY);
